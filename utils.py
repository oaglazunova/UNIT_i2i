...
class RandomFixedRotation:
    ...
    def __call__(self, img):
        ...
        return torch.rot90(img, k=angle // 90, dims=[1, 2])


# Define your transformations
unit_transformations = transforms.Compose([
...
unit_normalize = transforms.Compose([
    transforms.Normalize(0.5, 0.5),
])
...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None, height=256, width=256, num_workers=4,
                         crop=True):
...
def load_vgg16(model_dir):
    ...
    if not os.path.exists(os.path.join(model_dir, 'vgg16.weight')):
        ...
        vgglua = torch(os.path.join(model_dir, 'vgg16.t7'))
        vgg = Vgg16()
